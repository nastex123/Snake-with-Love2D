-- systems/debugTools.lua - Menu debug Tab + modal logros (logo F2 delegado a debugLogo.lua)
local debugTools = {}
...
function debugTools.mousepressed(x, y, button)
    if debugLogo.mousepressed(x, y, button) then return true end
    local st = world.state
...
function debugTools.mousemoved(x, y, dx, dy)
    if debugLogo.mousemoved(x, y) then return true end
    return false
end

function debugTools.mousereleased(x, y, button)
    if debugLogo.mousereleased() then return true end
    return false
end
