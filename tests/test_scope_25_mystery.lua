...
harness.describe("Scope 25 - Doppelganger (mirror, delay, feed)", function()
    harness.it("beginDoppel mirrors the body across the center", function()
        mystery.begin()
        local dd = mystery.beginDoppel({{x = 1, y = 2}, {x = 0, y = 2}}, 10, 8)
        harness.assert_equal(2, #dd.body, "same length")
        harness.assert_equal(8, dd.body[1].x, "mirrored x")
        harness.assert_equal(5, dd.body[1].y, "mirrored y")
        harness.assert_equal(0, dd.fed, "fed starts at zero")
        mystery.begin()
    end)

    harness.it("doppelTick delays replication by 1.2s", function()
        mystery.begin()
        local dd = mystery.beginDoppel({{x = 5, y = 5}}, 10, 8)
        harness.assert_equal(4, dd.body[1].x, "body mirrored on spawn")
        mystery.doppelTick(0.13, dd, {x = 1, y = 0}, 0.0, 0.13)
        harness.assert_equal(4, dd.body[1].x, "holds during delay")
        mystery.doppelTick(0.13, dd, {x = 1, y = 0}, 2.0, 0.13)
        harness.assert_equal(5, dd.body[1].x, "moves after delay")
        harness.assert_equal(1, #dd.body, "length preserved")
        mystery.begin()
    end)

    harness.it("doppelTouchesHead detects overlap only", function()
        local dd = {body = {{x = 3, y = 3}, {x = 2, y = 3}}}
        harness.assert_true(mystery.doppelTouchesHead(dd, {x = 2, y = 3}), "tail overlap kills")
        harness.assert_true(not mystery.doppelTouchesHead(dd, {x = 9, y = 9}), "far is safe")
        harness.assert_true(not mystery.doppelTouchesHead(nil, {x = 3, y = 3}), "nil-safe")
    end)

    harness.it("doppelFed dissolves on the third normal fruit", function()
        mystery.begin()
        harness.assert_nil(mystery.doppelFed(), "no entity, no count")
        mystery.beginDoppel({{x = 1, y = 1}}, 10, 8)
        harness.assert_nil(mystery.doppelFed(), "first fruit")
        harness.assert_nil(mystery.doppelFed(), "second fruit")
        harness.assert_equal("dissolve", mystery.doppelFed(), "third dissolves")
        mystery.begin()
    end)

    harness.it("doppelHead returns the leading segment", function()
        mystery.begin()
        harness.assert_nil(mystery.doppelHead(), "none active")
        mystery.beginDoppel({{x = 7, y = 2}}, 10, 8)
        local h = mystery.doppelHead()
        harness.assert_equal(2, h.x, "mirrored head x")
        harness.assert_equal(5, h.y, "mirrored head y")
        mystery.begin()
    end)
end)

harness.describe("Scope 25 - Trial of Triads (order, timer)", function()
    harness.it("triadsRunes places 1-2-3 in three corners", function()
        local runes = mystery.triadsRunes(12, 10)
        harness.assert_equal(3, #runes, "three runes")
        harness.assert_equal(1, runes[1].x, "rune 1 corner")
        harness.assert_equal(1, runes[1].y, "rune 1 corner")
        harness.assert_equal(10, runes[2].x, "rune 2 corner")
        harness.assert_equal(8, runes[3].y, "rune 3 corner")
    end)

    harness.it("triadsStep advances only in order and on entry", function()
        mystery.begin()
        local tr = mystery.beginTriads(12, 10)
        harness.assert_equal(10.0, tr.timer, "10s timer")
        harness.assert_equal("next", mystery.triadsStep(tr, {x = 1, y = 1}), "first seal")
        harness.assert_nil(mystery.triadsStep(tr, {x = 1, y = 1}), "standing still retriggers nothing")
        harness.assert_equal("reset", mystery.triadsStep(tr, {x = 1, y = 8}), "wrong order resets")
        harness.assert_equal(0, tr.progress, "progress cleared")
        harness.assert_equal("next", mystery.triadsStep(tr, {x = 5, y = 5}) or mystery.triadsStep(tr, {x = 1, y = 1}), "restarts")
        harness.assert_equal("next", mystery.triadsStep(tr, {x = 10, y = 1}), "second seal")
        harness.assert_equal("complete", mystery.triadsStep(tr, {x = 1, y = 8}), "third completes")
        harness.assert_true(tr.done, "sealed done")
        harness.assert_nil(mystery.triadsStep(tr, {x = 1, y = 1}), "done ignores steps")
        mystery.begin()
    end)

    harness.it("triadsActive gates on room and completion", function()
        mystery.begin()
        local fake = {getCurrentRoom = function() return {mystery = "trial_triads"} end}
        harness.assert_true(not mystery.triadsActive(fake), "no runtime, inactive")
        mystery.beginTriads(12, 10)
        harness.assert_true(mystery.triadsActive(fake), "active with runtime")
        local plain = {getCurrentRoom = function() return {} end}
        harness.assert_true(not mystery.triadsActive(plain), "normal room inactive")
        mystery.begin()
    end)
end)
